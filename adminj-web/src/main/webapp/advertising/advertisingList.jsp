	...
	<script type="text/javascript">
		$(function() {
			$('#dg').datagrid( {
				title:'广告列表',
				align:'center',
				url:'${ctx }/advertising/advertisingDataList.json',
				singleSelect:true,
				pagination:true,
				pageSize:10,
				...
				columns : [[
					{
						...
						field : 'advertisingLogo',
						title : '广告logo',
						align:'center',
						width : 140,
						formatter: function(value,row,index){
							return '<img id="prizeTableImg"  style="width:100px;height:80px" src="'+${ctx}value+'"/>';
						}
					},{
						field : 'createTime',
						title : '创建日期',
						...
						width : 200,
						formatter: function(value,row,index){
							return "<a href='${ctx }/advertising/checkAdvertising.html?advertisingId="+row['advertisingId']+"'>预览</a>";
						}
					}
				]]
			});
			...
			$("#queryBtn").click(function(){
				$('#dg').datagrid('load',{
					advertisingUrl : $("#advertisingUrl").textbox("getValue"),
			...
			$("#addBtn").click(function(){
				window.location = "${ctx }/advertising/toinsertAdvertising.html";
				return false;
			});
			...
			$("#updateBtn").click(function(){
				...
				if(items == null || items.length == 0){
					...
					window.location = "${ctx}/advertising/preUpdateAdvertising.html?advertisingId=" + items[0]['advertisingId'];
				}
				return false;
			});
			...
			$("#deleteBtn").click(function(){
				...
				if(items == null || items.length == 0){
					...
					$.messager.confirm('确认提示', '确定要删除记录吗？', function(r){
						if(r){
							var url = "${ctx}/advertising/delAdvertising.json";
							$.post(url,{advertisingId:items[0]['advertisingId']}, function(data){
								if(data){
									if(data["code"]==200){
...
<div class="bg1">
	<div class="easyui-panel" title="查询条件" style="padding:7px 10px; margin-bottom: 10px;">
		<div id="tb" style="padding:2px 5px;">
			广告url： <input type="text" name="advertisingUrl" id="advertisingUrl" class="easyui-textbox" style="width:200px;"/>
			状态：
